...
$(function () {
    ...
    function os_init_customizer() {
        ...
        if (layout = localStorage.getItem('menu-layout-selector')){
            $('.menu-layout-selector').val(layout)
        }
        if (image = localStorage.getItem('with-image-selector')){
            $('.with-image-selector').val(image)
        }
        if (menu_color = localStorage.getItem('menu-color-selector')){
            var menu_selector = $('.menu-color-selector .'+menu_color);
            menu_selector.addClass('selected');
            menu_selector.siblings().removeClass('selected');
        }
        if (menu_style = localStorage.getItem('sub-menu-style-selector')){
            $('.sub-menu-style-selector').val(menu_style)
        }
        if (sub_menu = localStorage.getItem('sub-menu-color-selector')){
            var sub_menu_selector = $('.sub-menu-color-selector .'+sub_menu);
            sub_menu_selector.addClass('selected');
            sub_menu_selector.siblings().removeClass('selected');
        }
        if (fullscreen = localStorage.getItem('full-screen-selector')){
            $('.full-screen-selector').val(fullscreen)
        }
        if (top_bar_visibility = localStorage.getItem('top-bar-visibility-selector')){
            $('.top-bar-visibility-selector').val(top_bar_visibility)
        }
        if (top_bar_above_menu = localStorage.getItem('top-bar-above-menu-selector')){
            $('.top-bar-above-menu-selector').val(top_bar_above_menu)
        }
        if (top_bar_color = localStorage.getItem('top-bar-color-selector')){
            var top_bar_color_selector = $('.top-bar-color-selector .'+top_bar_color);
            top_bar_color_selector.addClass('selected');
            top_bar_color_selector.siblings().removeClass('selected');
        }
    ...
    function os_apply_customizations() {
        ...
        if (menu_layout == 'mini') {
            ...
            if (menu_position == 'left') {
                $('.menu-actions .os-dropdown-trigger').removeClass(function (index, className) {
                    return (className.match(/(^|\s)os-dropdown-position-\S+/g) || []).join(' ');
                });
